...
ui <- fluidPage(
        ...
        sidebarLayout(
                sidebarPanel(
                        selectInput("ticker", "Select Ticker:", choices = c("AAPL", "AMD", "GOOG", "AMZN", "TSLA")),
                        selectInput("date_length", "Select Date Length:", choices = c("1y", "2y", "3y", "4y", "5y")),
                        selectInput("analysis_method", "Select Analysis Method:", choices = c("ATR", "Rolling Momentum", "Relative Strength Index")),
                        actionButton("calculate", "Calculate"),
                        themeSelector()
                ),
                # Display plots in main panel
                mainPanel(
        ...
        # Custom CSS for main panel positioning
        tags$style(HTML("
            .custom-main-panel {
...
server <- function(input, output) {
        ...
        output$price_plot <- renderPlot({
                req(stock_data()) # Ensure the data is available before plotting
                selected_data <- stock_data()
                plot(selected_data$date, selected_data$close, type = "l", main = "Stock Price", xlab = "Date", ylab = "Price")
        })

        # Render analysis plot based on method
        output$analysis_output <- renderPlot({
                req(stock_data(), selected_analysis$method) # Ensure the data and analysis method are available before plotting
                selected_data <- stock_data()
                ...
                if (selected_analysis$method == "ATR") {
                        ...
                        plot(index(selected_xts), atr$atr, type = "l", main = "Average True Range", xlab = "Date", ylab = "ATR")
                }  else if (selected_analysis$method == "Rolling Momentum") {
                        rolling_momentum <- lag(selected_xts$close, 12) / selected_xts$close - 1
                        plot(index(selected_xts), rolling_momentum, type = "l", main = "Rolling Momentum", xlab = "Date", ylab = "Momentum")

                } else if (selected_analysis$method == "Relative Strength Index") {
                        rsi <- RSI(selected_xts$close, n = 14, maType = "SMA")
                        plot(index(selected_xts), rsi, type = "l", main = "Relative Strength Index", xlab = "Date", ylab = "RSI")
                }
        })
}
